  ...
  let query_single_day_revenue = () => {
    fetch('report_php/single_day_revenue.php', {
      ...
      .then((myJson) => {
        ...
        let canvas_box = document.getElementById('single_day_canvas_box')
        canvas_box.innerHTML = '<canvas id="single_day_chart"></canvas>'

        let ctx = document.getElementById('single_day_chart').getContext('2d')
        let single_day_chart = new Chart(ctx, {
          type: 'bar',
          data: {
            labels: date_range,
            datasets: [{
